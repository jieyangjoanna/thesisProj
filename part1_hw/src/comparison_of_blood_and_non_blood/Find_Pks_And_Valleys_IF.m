function [pks, pks_locs, valleys, valy_locs] = Find_Pks_And_Valleys_IF( filename, wavelength_begin, wavelength_end )
...
if isempty(pks_locs)
    pks=max(spectrum((wavelength_begin - 350+1):(wavelength_end-350+1)));
    pks_locs = wavelength_begin + find(spectrum( (wavelength_begin - 350+1):(wavelength_end-350+1)) == pks) -1;
end
...
pks_locs = pks_locs + BS_Ave;% If work with IF; pks_locs should shift
pks = spectrum(pks_locs-350+1);
...
valy_locs = valy_locs + BS_Ave;%If work with IF; pks_locs should shift
valleys = spectrum(valy_locs-350+1);
