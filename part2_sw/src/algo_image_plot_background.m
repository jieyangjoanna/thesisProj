function [TPR_p0001_oneSubstrate_allAlgos, AUC_PR_oneSubstrate_allAlgos, AUC_ROC_oneSubstrate_allAlgos]= algo_image_plot_background(I, ROI, idx_bckgrd, t, GT, substrate)
...
%   idx_bckgrd is the indicies of detected background pixels in ROI.
%   the dimension is background_pixel_# - by - 1. 
%   For example, GF substrate, the idx_bckgrd = 69296 x 1. 
...
for i = 1:length(algos)
    ...
    if strcmp(algo,'MF')
        ...
        [MF_score_global] = MF_bckground(I_ROI, idx_bckgrd, t');
    ...
    elseif strcmp(algo, 'CEM')
        ...
        % CEM_score = CEM(I_ROI,t'); %CEM(I_ROI,t_BCInner');
         CEM_score = CEM_bckground(I_ROI, idx_bckgrd, t');
        if min(CEM_score)<0
    ...
    elseif strcmp(algo, 'ACE')
        ...
        ACE_score = ACE_bckground(I_ROI, idx_bckgrd, t'); 
        if min(ACE_score)<0
    ...
    ALGO_non_GT_data = ALGO_image_0(setdiff(idx_bckgrd_im, idx_GT));
   ...
   % clearvars idx_bigger_than_p0001 idx_smaller_than_p0001 TPR_p0001
    ...
    legend(h1,'MF','CEM','ACE','SAM', 'Location', 'Best');
    legend(h2,'MF','CEM','ACE', 'SAM', 'Location', 'Best');
    plot(h2, 0:0.01:1, 0:0.01:1, 'k-.', 'linewidth', 1.5,'HandleVisibility','off');


saveas(h1,strcat(result_folder, strcat('PR Using In-scene BloodstainFullPixel on_', substrate, '_bckground.png')))
saveas(h2,strcat(result_folder, strcat('ROC Using In-scene BloodstainFullPixel on_', substrate, '_bckground.png')))
